...
export const createGraph2DTool: Tool = {
  ...
  execute: async (args: any): Promise<ExecuteToolResult> => {
    try {
      ...
      // Simple function parser (very limited, just for demo)
      const fn = expression.replace('y=', '').replace('f(x)=', '');
      
      // Generate data points
      for (let x = xMin; x <= xMax; x += step) {
        try {
          // DANGER: This is just for demonstration and is NOT safe for production
          // In a real implementation, use a proper math expression parser
          // eslint-disable-next-line no-eval
          const y = eval(fn.replace(/x/g, `(${x})`).replace(/\\^/g, '**'));
          dataPoints.push({ x, y });
        } catch (error) {
